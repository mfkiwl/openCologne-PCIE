# _openCologne/PCIE_ Build

The build process for the _openCologne/PCIE_ project consists of three steps:
- Compilation of _openCologne/PCIE_ control and status registers (CSR) from RDL specification into RTL for hardware design and harware abstraction layer (HAL) for software application
...
To generate all the required HAL headers and RTL a make file is provided as `4.build/MakefileCSR` to wrap app the script calls. Running this make file (`make -f MakefileCSR`) produces the following files in `4.build/csr_build/generated-files`:
  ...
  * `csr_cosim.h`   : The HAL for the *VProc* based __openCologne/PCIE___ logic simulation test bench
...
#endif
```
This has been done in the `openCologne/PCIE_regs.h` header in the `4.build/csr_build/generated-files/` directory, and including this header in the application code makes available the register HAL.
...
Hardware synthesis is currently supported through the Vivado GUI, using the prepared [project file](/4.build/hw_build.Vivado/openCologne/PCIE.xpr) located at `4.build/hw_build.Vivado`.